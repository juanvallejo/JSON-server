/**
 * UAS modules. Provide wrappers with predefined values, classes, etc. for test cases
 */
...
require('fs').readdirSync(__dirname).forEach(function(module) {
	...
	// make sure we are filtering out system files
	if(module != 'index.js' && module.split('js').length > 1) {
		
		currentModule = require(__dirname + '/' + module);
