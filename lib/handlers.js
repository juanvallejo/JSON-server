...
Handlers.handleRequestAsUASAPICall = function(request, response) {
	...
	request.on('end', function() {
		...
		if(!(request.headers && request.headers['uasapi-method'])) {
		...
		if(request.headers['uasapi-method'] == 'GET') {
			
			if(UAS.authenticationRequestCookies) {
				console.log('COOKIES FOUND');
			}

			return UAS.handleGetRequest(request, function(data) {
				Responses.respondOKWithMessage(response, data);
			});

		}

		// assume the uas-api method is of type POST
		UAS.handlePostRequest(request, postData, function(APIResponse, APIResponseCookies) {
